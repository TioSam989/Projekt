...
if($mysqli){
        ...
        if(!empty($res = mysqli_fetch_array($result))){
            ...
            <?php include("../pages/headerUSERS.html"); ?>
            ...
            <div class="login">
							...
							<form class="login-form" method="post" action="">
								...
								<div class="row">
									...
									<div class="row">
										<div class="col">
											<label>Turma<i class="fas fa-question-circle fa-xs" title="Deve conter de 3-20 caracteres e deve conter no minimo um caractere maiusculo, minusculo e numero"></i>
												<input type="text" name="username" id="username" value="<?php echo $res['turma'] ?>" pattern="^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])(?=\S+$).{3,20}$" class="Tinput" disabled>
											</label>
										</div>
									</div>
                                <center>
                                    ...
                                    <label  style="background-color:red" class="big-btn">
										...
										$id=$res['idLogin'];
											if ((utf8_decode($res['ativo'])=='N') || (utf8_decode($res['ativo'])=='Z')){
												echo "<a class='activate'style='display:block;padding:0.5rem' href=\"../ADMpages/ativar.php?id=$id\" onClick=\"return confirm('Tem a certeza que quer ativar esse usuario ?')\">ativar</a>";	
											}else{
												echo "<a class='deactivate' style='display:block;padding:0.5rem' href=\"../ADMpages/desativar.php?id=$id\" onClick=\"return confirm('Tem a certeza que quer desativar esse usuario ?')\">desativar</a>";	
											}
										?>
									</label>
