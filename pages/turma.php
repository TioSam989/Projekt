...
<?php
	...
	$result = mysqli_query($mysqli , "SELECT * FROM turma WHERE sigla_Turma='$materia'");
							...
							<form class="login-form" method="post" style="border-top-left-radius:0;border-top-right-radius:0; " action="">
								...
								<div class="row">
									...
									<div class="col"> 
										...
										<?php 
											$cv =  mysqli_query($mysqli , "SELECT * FROM login where turma='$materia'  ");
											$alunos = mysqli_fetch_assoc($resucvltado);

											if(!empty($ros = mysqli_fetch_array($cv))){
											
												$cv =  mysqli_query($mysqli ,"SELECT * FROM login WHERE turma='$materia' and ativo='S'");
												$ada =  mysqli_query($mysqli ,"SELECT * FROM login WHERE turma='$materia' and ativo!='S'");
												...
												while($res = mysqli_fetch_array($ada)) {	
													...
													echo "<a href='profile.php?username=".$res['username']."'>";
													echo "<div class='col'>";
													...
													echo "<button>"."<a class='activate' href=\"../ADMpages/ativar.php?id=$id\" onClick=\"return confirm('Tem a certeza que quer ativar esse usuario ?')\">ativar</a>"."</button>";
													echo "</center>";
													echo "</div>";
													echo "</a>";
												}
												while($res = mysqli_fetch_array($cv)) {	
													...
													echo "<h3 style='text-elign:center; margin-bottom:0;margin-top:opx' class='Tinput'>$nome</h3>";
													echo "<h5 style='text-elign:center;margin-top:opx; border-top:0px; padding-top:0px' class='Tinput'>$apelido</h5>";
													echo "<button>"."<a class='deactivate' href=\"../ADMpages/desativar.php?id=$id\" onClick=\"return confirm('Tem a certeza que quer desativar esse usuario ?')\">desativar</a>"."</button>";
													echo "</center>";
													echo "</div>";
													echo "</a>";
												}
