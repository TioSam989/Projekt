...
$resultado = mysqli_query($mysqli, "SELECT * FROM login where codigoMensagem!='' ");

if (isset($_POST['procura'])){
	$valor_a_procurar=$_POST['id'];
	$resultado = mysqli_query($mysqli, "SELECT * FROM login where codigoMensagem!='' AND nome like'%".$valor_a_procurar."%' OR email LIKE'%".$valor_a_procurar."%' OR username LIKE'%".$valor_a_procurar."%' OR turma LIKE'%".$valor_a_procurar."%' OR sexo LIKE'%".$valor_a_procurar."%' OR born LIKE'%".$valor_a_procurar."%'  ");
}
...
?>
	<div class="login">
        <form action="#" method="POST" class="login-form">
			...
			<center><h2>Todos usuarios</h2></center>
            ...
            if(is_array($linha) && !empty($linha)){
				...
				echo "<div class='cow'>";
				echo "<a href='./lerMensagem.php?code=".$linha['codigoMensagem']."'>";
		...
		echo "<div>";
		echo "<center>";
		echo "<a class='row moh' href='./readMessages.php'> ";
		echo "<div class='cow'>";
		echo "<label>";
		echo "<i class='fas fa-users  fa-5x'></i>";
		echo "mensagem Geral </a>";
		echo "<h1>Click</h1>";
		echo "</label>";
		echo "</div>";
		echo "</div>";
